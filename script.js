...
let macroFallbackCache = null;
let firmFallbackCache = null;
...
function toggleFullscreen() {
    ...
    if (!isFullscreen) {
        ...
        // Trigger chart resize after entering fullscreen
        setTimeout(() => {
            try { if (macroChartInstance) macroChartInstance.resize(); } catch(_){}
            try { if (firmChartInstance) firmChartInstance.resize(); } catch(_){}
        }, 250);
    } else {
        try {
        ...
        // Trigger chart resize after exiting fullscreen
        setTimeout(() => {
            try { if (macroChartInstance) macroChartInstance.resize(); } catch(_){}
            try { if (firmChartInstance) firmChartInstance.resize(); } catch(_){}
        }, 250);
...
// Basic swipe navigation for mobile devices
...
function handleTouchEnd(e){
    ...
    if(Math.abs(dx) > Math.abs(dy) && Math.abs(dx) > swipeThreshold){
        if(dx < 0){ changeSlide(1); } else { changeSlide(-1); }
    }
...
function initMacroChart() {
        ...
        macroChartInstance = new Chart(ctx, {
            ...
            options: {
                ...
                scales: {
                    ...
                    y: { type: 'linear', position: 'left', grid: { color: 'rgba(229,238,246,.7)' }, title: { display: true, text: 'GDP Growth (%) / Price Index' } },
    ...
    function datasetFromCSV(rows) {
        ...
        const gdpGrowth = output.map((v, i) => {
            if (i === 0) return null;
            const prev = output[i - 1];
            if (prev == null || prev === 0 || v == null) return null;
            return ((v / prev) - 1) * 100;
        });
        return {
            labels,
            datasets: [
                { label: 'GDP Growth (%)', data: gdpGrowth, borderColor: '#2ecc71', backgroundColor: '#2ecc71', tension: .2, yAxisID: 'y', pointStyle: 'circle', pointRadius: 3, pointHoverRadius: 4, pointBackgroundColor: '#2ecc71', pointBorderColor: '#2ecc71' },
                { label: 'Price Index', data: price, borderColor: '#e67e22', backgroundColor: '#e67e22', tension: .2, yAxisID: 'y', pointStyle: 'circle', pointRadius: 3, pointHoverRadius: 4, pointBackgroundColor: '#e67e22', pointBorderColor: '#e67e22' },
                { label: 'Unemployment (%)', data: unemp, borderColor: '#3498db', backgroundColor: '#3498db', tension: .2, yAxisID: 'y1', pointStyle: 'circle', pointRadius: 3, pointHoverRadius: 4, pointBackgroundColor: '#3498db', pointBorderColor: '#3498db' }
            ]
        };
    }

    function sliceTo36(data) {
        try {
            const n = Array.isArray(data.labels) ? data.labels.length : 0;
            if (n <= 36) return data;
            return {
                labels: data.labels.slice(0, 36),
                datasets: data.datasets.map(ds => ({
                    ...ds,
                    data: Array.isArray(ds.data) ? ds.data.slice(0, 36) : ds.data
                }))
            };
        } catch (_) { return data; }
    }

    function getMacroFallbackDataset() {
        if (macroFallbackCache) return macroFallbackCache;
        ...
        try {
            ...
            const dataset = {
                ...
                datasets: [
                    { label: 'GDP Growth (%)', data: payload.series.GDPGrowth, borderColor: '#2ecc71', backgroundColor: '#2ecc71', tension: .2, yAxisID: 'y', pointStyle: 'circle', pointRadius: 3, pointHoverRadius: 4, pointBackgroundColor: '#2ecc71', pointBorderColor: '#2ecc71' },
                    { label: 'Inflation (%)', data: payload.series.Inflation, borderColor: '#e67e22', backgroundColor: '#e67e22', tension: .2, yAxisID: 'y', pointStyle: 'circle', pointRadius: 3, pointHoverRadius: 4, pointBackgroundColor: '#e67e22', pointBorderColor: '#e67e22' },
                    { label: 'Unemployment (%)', data: payload.series.Unemployment, borderColor: '#3498db', backgroundColor: '#3498db', tension: .2, yAxisID: 'y1', pointStyle: 'circle', pointRadius: 3, pointHoverRadius: 4, pointBackgroundColor: '#3498db', pointBorderColor: '#3498db' }
                ]
            };
            macroFallbackCache = dataset;
            // Reduce exposure of embedded data after parsing
            try { dataEl.textContent = ''; dataEl.setAttribute('data-fallback-cleared','1'); if (dataEl.parentNode) dataEl.parentNode.removeChild(dataEl); } catch (_) {}
            return dataset;
        } catch (_) { return null; }
    }

    // Fetch the single timestamped CSV named in hint file; fallback to external JSON payload
    resolveCsvViaHint('macro_indicators')
        ...
        .then(rows => {
            const csvData = datasetFromCSV(rows);
            const fallback = getMacroFallbackDataset();
            const isCsv36 = Array.isArray(csvData.labels) && csvData.labels.length >= 36;
            const finalData = isCsv36 ? sliceTo36(csvData) : (fallback || csvData);
            if (!isCsv36 && fallback) console.log('[MacroChart] CSV had <36 periods; using draft fallback demo data');
            render(finalData);
        })
        .catch((err) => {
            console.warn('CSV fetch failed:', err.message);
            const cached = getMacroFallbackDataset();
            if (cached) { render(sliceTo36(cached)); return; }
            // Load external fallback JSON
            fetch('./assets/fallback/macro_indicators_fallback.json', { cache: 'no-store' })
                ...
                .then(payload => {
                    ...
                    render(sliceTo36(macroFallbackCache));
                })
                .catch(() => console.warn('External fallback JSON failed'));
        });
}
...
function initFirmDynamicsChart() {
        ...
        firmChartInstance = new Chart(ctx, {
            ...
            options: {
                ...
                scales: {
                    ...
                    y2: { type: 'linear', position: 'right', offset: true, grid: { drawOnChartArea: false }, title: { display: true, text: 'Energy behavior (% / Index)' } }
                },
                plugins: {
                    legend: {
                        display: true,
    ...
    function aggregateFirmCSV(rows) {
        ...
        return { labels, datasets: [
            { label: 'Avg Output per Firm', data: outputAvg, borderColor: '#2ecc71', backgroundColor: '#2ecc71', tension: .2, yAxisID: 'y', pointStyle: 'circle', pointRadius: 3, pointHoverRadius: 4, pointBackgroundColor: '#2ecc71', pointBorderColor: '#2ecc71' },
            { label: 'Avg Unit Cost', data: ucAvg, borderColor: '#e67e22', backgroundColor: '#e67e22', tension: .2, yAxisID: 'y', pointStyle: 'circle', pointRadius: 3, pointHoverRadius: 4, pointBackgroundColor: '#e67e22', pointBorderColor: '#e67e22' },
            { label: 'Avg Delivered Output', data: delivAvg, borderColor: '#3498db', backgroundColor: '#3498db', tension: .2, yAxisID: 'y1', pointStyle: 'circle', pointRadius: 3, pointHoverRadius: 4, pointBackgroundColor: '#3498db', pointBorderColor: '#3498db' }
        ]};
    }
    ...
    function getFirmFallbackDataset() {
        if (firmFallbackCache) return firmFallbackCache;
        ...
        try {
            const payload = JSON.parse(dataEl.textContent);
            const datasets = [
                { label: 'Avg Output per Firm', data: payload.series.OutputAvg, borderColor: '#2ecc71', backgroundColor: '#2ecc71', tension: .2, yAxisID: 'y', pointStyle: 'circle', pointRadius: 3, pointHoverRadius: 4, pointBackgroundColor: '#2ecc71', pointBorderColor: '#2ecc71' },
                { label: 'Avg Unit Cost', data: payload.series.AvgUCAvg, borderColor: '#e67e22', backgroundColor: '#e67e22', tension: .2, yAxisID: 'y', pointStyle: 'circle', pointRadius: 3, pointHoverRadius: 4, pointBackgroundColor: '#e67e22', pointBorderColor: '#e67e22' },
                { label: 'Avg Delivered Output', data: payload.series.OutDelivAvg, borderColor: '#3498db', backgroundColor: '#3498db', tension: .2, yAxisID: 'y1', pointStyle: 'circle', pointRadius: 3, pointHoverRadius: 4, pointBackgroundColor: '#3498db', pointBorderColor: '#3498db' }
            ];
            if (payload.series && Array.isArray(payload.series.EnergyPriceIdx)) {
                datasets.push({ label: 'Energy Price Index', data: payload.series.EnergyPriceIdx, borderColor: '#8e44ad', backgroundColor: '#8e44ad', tension: .2, yAxisID: 'y2', pointStyle: 'circle', pointRadius: 3, pointHoverRadius: 4, pointBackgroundColor: '#8e44ad', pointBorderColor: '#8e44ad' });
            }
            if (payload.series && Array.isArray(payload.series.EfficiencyInvestmentRate)) {
                datasets.push({ label: 'Efficiency Investment Rate (%)', data: payload.series.EfficiencyInvestmentRate, borderColor: '#1abc9c', backgroundColor: '#1abc9c', tension: .2, yAxisID: 'y2', pointStyle: 'circle', pointRadius: 3, pointHoverRadius: 4, pointBackgroundColor: '#1abc9c', pointBorderColor: '#1abc9c' });
            }
            if (payload.series && Array.isArray(payload.series.OnsiteGenerationShare)) {
                datasets.push({ label: 'On-site Generation Share (%)', data: payload.series.OnsiteGenerationShare, borderColor: '#f43f5e', backgroundColor: '#f43f5e', tension: .2, yAxisID: 'y2', pointStyle: 'circle', pointRadius: 3, pointHoverRadius: 4, pointBackgroundColor: '#f43f5e', pointBorderColor: '#f43f5e' });
            }
            const dataset = { labels: payload.labels, datasets };
            firmFallbackCache = dataset;
            // Reduce exposure of embedded data after parsing
            try { dataEl.textContent = ''; dataEl.setAttribute('data-fallback-cleared','1'); if (dataEl.parentNode) dataEl.parentNode.removeChild(dataEl); } catch (_) {}
            return dataset;
        } catch (_) { return null; }
    ...
    // Fetch the single timestamped CSV named in hint file; fallback to external JSON
    resolveCsvViaHint('sample_firms_production')
        .then(csvUrl => fetch(csvUrl, { cache: 'no-store', mode: 'cors', credentials: 'same-origin' }))
        .then(r => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.text(); })
        .then(text => parseCSV(text))
        .then(rows => {
            const csvData = aggregateFirmCSV(rows);
            const fallback = getFirmFallbackDataset();
            const isCsv36 = Array.isArray(csvData.labels) && csvData.labels.length >= 36;
            const finalData = isCsv36 ? sliceTo36(csvData) : (fallback || csvData);
            if (!isCsv36 && fallback) console.log('[FirmChart] CSV had <36 periods; using draft fallback demo data');
            render(finalData);
        })
        .catch(() => {
            const cached = getFirmFallbackDataset();
            if (cached) { render(sliceTo36(cached)); return; }
            // Load external fallback JSON
            fetch('./assets/fallback/firm_dynamics_fallback.json', { cache: 'no-store' })
                .then(r => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.json(); })
                .then(payload => {
                    const datasets = [
                        { label: 'Avg Output per Firm', data: payload.series.OutputAvg, borderColor: '#2ecc71', backgroundColor: '#2ecc71', tension: .2, yAxisID: 'y', pointStyle: 'circle', pointRadius: 3, pointHoverRadius: 4, pointBackgroundColor: '#2ecc71', pointBorderColor: '#2ecc71' },
                    ...
                    firmFallbackCache = { labels: payload.labels, datasets };
                    render(sliceTo36(firmFallbackCache));
                })
                .catch(() => {});
        });
}
...
(function registerGlobalTooltips() {
    ...
    window.initDecisionProcessWidget = function initDecisionProcessWidget() {
        ...
        function updateDecisionProcess() {
            ...
            const energyPlantRow = container.querySelector('[data-type="open-energy-plant"]');
            ...
            const energyPlantCost = getCost('open-energy-plant', 20);

            // Calculate energy plant NPV based on energy price and interest rate
            const energyPlantNPV = (energyPrice > 100 ? 22.0 + (energyPrice - 100) * 0.15 : 18.0) * (1 - (interestRate - 1) * 0.03);
            ...
            const investments = [
                ...
                { type: 'open-energy-plant', cost: energyPlantCost, npv: energyPlantNPV, row: energyPlantRow }
            ...
            sequenceTimers.push(setTimeout(() => {
                ...
                if (energyPlantRow) energyPlantRow.querySelector('.dp-npv').textContent = energyPlantNPV.toFixed(1);
                proposalsCount.textContent = '4';
            ...
            sequenceTimers.push(setTimeout(() => {
                [efficiencyRow, capacityRow, renewalRow, energyPlantRow].forEach(r => {
                    if (!r) return;
